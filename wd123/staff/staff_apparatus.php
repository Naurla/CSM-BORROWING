...
// Initialize values (ADDED $damaged_stock and $lost_stock for forms)
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['add_apparatus'])) {
    ...
    $damaged_stock = (int)$_POST['damaged_stock']; // NEW: CAPTURE DAMAGED STOCK
    $lost_stock = (int)$_POST['lost_stock']; // NEW: CAPTURE LOST STOCK
...
// 5. NEW: Handle Unit Restoration from Unit Management Modal
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['unit_to_restore'])) {
    ...
    $result = $transaction->restoreDamagedUnit($unit_id, $staff_id);
    
    if ($result === true) {
        $message = "✅ Unit ID {$unit_id} successfully restored to good condition and made available.";
    } elseif ($result === 'not_damaged') {
        $message = "❌ Error: Unit ID {$unit_id} was not marked as damaged or does not exist.";
    } else {
...
<head>
    ...
    <style>
        ...
        .table tbody td {
            ...
            font-size: 0.9rem;
            text-align: center;
            /* Added min-width for key descriptive columns to prevent collapse */
            min-width: 60px; 
        }
        ...
        .status-tag.available { background-color: #28a74530; color: #1e7e34; }
        .status-tag.borrowed { background-color: #007bff30; color: #0056b3; }
        .status-tag.reserved { background-color: #ffc10740; color: #b8860b; }
        .status-tag.unavailable { background-color: #6c757d30; color: #5a6268; }
        .status-tag.damaged { background-color: #ffc10740; color: #b8860b; }
        .status-tag.good { background-color: #28a74530; color: #1e7e34; }
        
        /* --- START OF BUTTON UI IMPROVEMENTS --- */

        /* General container for all actions in a row */
        .action-buttons-group {
            display: flex;
            gap: 4px; /* Reduced gap for a tighter look */
            justify-content: center;
            align-items: center;
            flex-wrap: nowrap; /* Prevent wrapping in a single cell */
        }
        
        /* General styling for action buttons */
        .action-buttons-group .btn {
            padding: 6px 10px; /* Standardize padding for a slightly larger click area */
            font-size: 0.85rem;
            white-space: nowrap;
        ...
        /* Stock/Unit Management Buttons (Text + Icon) */
        .btn-stock-action {
            padding: 6px 8px; /* Slightly smaller to fit in the wider column */
        }

        /* Icon-Only Buttons (Edit and Delete) */
        .btn-icon-only {
            width: 35px; /* Fixed width/height for square icons */
            height: 35px;
            padding: 0;
            display: flex;
            justify-content: center;
        ...
        .btn-info:hover { 
            background-color: #138496; border-color: #138496; 
        }

        /* Edit (Warning) Icon */
        .btn-warning { 
            background-color: #ffc107; border-color: #ffc107; 
            color: #212529 !important; /* Ensure dark text on yellow */
        }
        .btn-warning:hover { 
        ...
        /* Re-adjust column widths for better fit */
        .table thead th:nth-child(16), 
        .table tbody td:nth-child(16) {
            width: 15%; /* Keep Stock/Unit Actions wider */
        }

        .table thead th:nth-child(17), 
        .table tbody td:nth-child(17) {
            width: 8%; /* Make Edit/Delete column smaller */
        }
        /* --- END OF BUTTON UI IMPROVEMENTS --- */
        
        /* Unit Management Modal Specific Styles */
        #unitManageModal .modal-header {
            background-color: var(--msu-blue);
            color: white;
        }
        #unitManageModal .modal-title {
            font-weight: 600;
        }
        /* Ensure table cells inside modal are readable */
        #unitManageModal table td, #unitManageModal table th {
            font-size: 0.85rem;
            text-align: center;
        }
        /* Restore Confirmation Modal */
        #restoreConfirmModal .modal-header {
...
<div class="modal fade" id="unitManageModal" tabindex="-1" aria-labelledby="unitManageModalLabel" aria-hidden="true">
    <div class="modal-dialog modal-dialog-scrollable modal-lg">
        <div class="modal-content">
            ...
            <div class="modal-body">
                <p class="text-muted small">Use the actions below to adjust the condition of damaged units (e.g., repair). Units currently marked **Damaged** or **Lost** are considered **Unavailable**.</p>
                <div class="table-responsive">
    ...
    <div class="modal-dialog modal-dialog-centered">
        <div class="modal-content">
            ...
            <div class="modal-body text-center">
                ...
                <p class="text-success fw-bold">This will decrement the Damaged count and increase the Available Stock.</p>
            </div>
                    ...
                    .then(data => {
                        ...
                        if (data.units && data.units.length > 0) {
                            data.units.forEach(unit => {
                                ...
                                } else if (unit.current_condition === 'lost') {
                                    buttonHtml = `<button class="btn btn-sm btn-danger" disabled>Lost</button>`;
                                } else {
